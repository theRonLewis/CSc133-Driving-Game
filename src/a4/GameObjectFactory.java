...
public class GameObjectFactory {
	...
	public GameObject getGameObject(String objType){
		...
		if(objType.equalsIgnoreCase("PLAYER")){
			playerOne = new PlayerCar(200, 200, Color.blue, 0, 0);
			return playerOne;
		} else if(objType.equalsIgnoreCase("NPC")){
			npcCounter++;
			switch(npcCounter){
			case 1: 
				npc1 = new NonPlayerCar(50, 200, NPC_COLOR, 0, 10);
				//npc1.setStrategy(new DestructionDerbyStrategy(npc1, playerOne));
				npc1.setStrategy(new RaceStrategy(npc1, myGameWorld));
				return npc1;
			case 2: 
				npc2 = new NonPlayerCar(125, 200, NPC_COLOR, 0, 10);
				npc2.setStrategy(new RaceStrategy(npc2, myGameWorld));
				return npc2;
			case 3: 
				npc3 = new NonPlayerCar(275, 200, NPC_COLOR, 0, 10);
				//npc3.setStrategy(new DestructionDerbyStrategy(npc3, playerOne));
				npc3.setStrategy(new RaceStrategy(npc3, myGameWorld));
				return npc3;
			}
			return null;
			...
			case 1: 
				return new Pylon(200, 200, PYLON_COLOR);
			case 2: 
				return new Pylon(200, 450, PYLON_COLOR);
			case 3: 
				return new Pylon(500, 580, PYLON_COLOR);
			case 4: 
				return new Pylon(650, 450, PYLON_COLOR);
			}
	...
	// support for dynamically adding objects at a given point from a click or collision
	public GameObject getGameObject(String objType, float xPos, float yPos){
		...
		if(objType.equalsIgnoreCase("PYLON")){
			...
			return new Pylon(xPos, yPos, new Color(64, 64, 64), inputNum );
			...
			return new FuelCan(xPos, yPos, inputNum);
			
		}else if(objType.equalsIgnoreCase("OIL")){
			// create and return the fuel can
			return new OilSlick(xPos, yPos);
		}else if(objType.equalsIgnoreCase("SHOCKWAVE")){
			// create and return new shock wave
			return new ShockWave(xPos, yPos);
		}
		return null;
	}
