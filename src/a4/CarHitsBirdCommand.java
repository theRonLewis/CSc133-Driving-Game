		...
		if(target != null){
			...
			if ( ((CollisionEvent)e.getSource()).getType().equals("CAR-BIRD") ){
				if(  ((CollisionEvent)e.getSource()).getFirst() instanceof Car )
					success = target.carBirdCollision( (Car)((CollisionEvent)e.getSource()).getFirst(), (Bird)((CollisionEvent)e.getSource()).getSecond() );
				else
					success = target.carBirdCollision( (Car)((CollisionEvent)e.getSource()).getSecond(), (Bird)((CollisionEvent)e.getSource()).getFirst() );
			}
			
			if(success) // play sound if went OK
				if( target.soundIsOn() )
					mySoundLib.playCarBirdCollisionSound();
